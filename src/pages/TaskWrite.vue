							...
							<div class="taskList taskLine-3">
								<ul>
									<li v-for="(ProductItem,index) in DataList">
										...
										<TaskWriteLi ref="TaskWriteLi" :ProductItem="ProductItem" :index="index" @hasFilled="hasFilled" @ShowRemarksBox="ShowRemarksBox"></TaskWriteLi>
									</li>
								</ul>
							</div>
						</div>
...
//					}]
		...
		methods: {
				...
				this.$ajax.getInitForm().then(res => {
					...
					for(var key in res.DataList) {
						if(res.DataList[key].price == 'null') {
							...
							res.DataList[key].waveReasonRemark = '';
						}
					}
			...
			//打开备注
			ShowRemarksBox: function(index , iswave = false) {
				this.iswave = iswave;
